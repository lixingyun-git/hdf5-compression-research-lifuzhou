...
HDF5CompressionModifier::HDF5CompressionModifier(const string &source_file, const string &dest_file, bool is_verbose) : inputFile(source_file), outputFile(dest_file), verbose(is_verbose) {}
HDF5CompressionModifier::~HDF5CompressionModifier() { closeFile(); }
...
bool HDF5CompressionModifier::openFile()
{   
    if (inputFile.empty() || outputFile.empty()) {
        cerr << "输入文件或输出文件未指定" << endl;
        return false;
    }

    inputFileId = H5Fopen(inputFile.c_str(), H5F_ACC_RDONLY, H5P_DEFAULT);
    if (inputFileId < 0)
    {
        cerr << "无法打开HDF5文件: " << inputFile << endl;
        return false;
    }

    outputFileId = H5Fcreate(outputFile.c_str(), H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
    // outputFile = H5::H5File(outputFile, H5F_ACC_TRUNC);
    if (outputFileId < 0)
    {
        cerr << "无法打开HDF5文件: " << outputFile << endl;
        return false;
    }
...
void HDF5CompressionModifier::closeFile()
{
    if (inputFileId != H5I_INVALID_HID)
    {
        H5Fclose(inputFileId);
        inputFileId = H5I_INVALID_HID;
    }

    if (outputFileId != H5I_INVALID_HID)
    {
        H5Fclose(outputFileId);
        outputFileId = H5I_INVALID_HID;
    }
}
...
bool HDF5CompressionModifier::getDatasets()
{
    std::vector<std::string> objects;
    datasets.clear();
    if (exploreFileStructure(objects) == false)
    {
        cerr << "无法获取数据集列表" << endl;
        return false;
    }
    ...
    for (const auto& obj : objects)
    {
        if (datasets.size() >= datasetLimit)
            break;
        if (obj.find("(Dataset)") != string::npos)
        {
            size_t pos = obj.find(" (Dataset)");
            string datasetName = obj.substr(0, pos);
...
// 修改压缩算法的主函数
bool HDF5CompressionModifier::modifyCompression(const string& datasetPath,
    ...
    int new_compressionLevel)
{
    ...
    // 读取现有数据
    if (!readHdf5Data(datasetPath, data, dataSize, dataRank))
    {
        return false;
    }

    // 使用新压缩算法保存
    if (!saveWithNewCompression(datasetPath, data, { dataSize }, filterType, new_compressionLevel))
    {
        return false;
    }
...
// 读取HDF5文件数据
bool HDF5CompressionModifier::readHdf5Data(const string& datasetPath, vector<int>& data,
    hsize_t& dataSize, int& dataRank)
{
    ...
    {
        // 打开数据集
        datasetId = H5Dopen(inputFileId, datasetPath.c_str(), H5P_DEFAULT);
        if (datasetId < 0)
        {
            cerr << "readHdf5Data 无法打开数据集: " << datasetPath << endl;
            return false;
        }
    ...
    unsigned int compressionLevel)
{
    ...
    try
    {
        ...
        dataspaceId = H5Screate_simple(dataRank, dims.data(), NULL);
        if (dataspaceId < 0)
        {
        ...
        datasetId = createDatasetInGroup(groupId, datasetFile, H5T_NATIVE_INT, dataspaceId, plistId);
        if (datasetId < 0)
        {
...
static herr_t visitCallback(hid_t locId, const char* name,
    const H5O_info_t* info, void* operatorData)
{
    vector<string>* objectList =
        static_cast<vector<string> *>(operatorData);
    ...
    string objectInfo = fullPath + " (" + typeStr + ")";
    objectList->push_back(objectInfo);

    return 0;
}

bool HDF5CompressionModifier::exploreFileStructure(vector<string> &objectList)
{
    herr_t status = H5Ovisit(inputFileId, H5_INDEX_NAME, H5_ITER_NATIVE,
        visitCallback, &objectList);

    if (status < 0)
    {
        cerr << "遍历HDF5文件结构失败" << endl;
        return false;
    }
